...
Key Features:
- Adaptive REST baseline: Adapts to fatigue during session (doesn't require returning to original baseline)
- Robust REST detection: Requires 3 consecutive REST predictions for reliable STOP
...
class AdaptiveBaselineTracker:
    """
    Tracks and adapts REST baseline during session to handle fatigue.
    Uses original REST baseline as reference but gradually adapts when REST is consistently detected.
    This allows the system to detect REST even when muscle activity doesn't return to original baseline.
    """
    
    def __init__(self, original_baseline: dict, adaptation_rate: float = 0.01):
        """
        Args:
            original_baseline: Original REST baseline from session start
            adaptation_rate: How quickly to adapt (0.01 = 1% per REST window, slow adaptation)
        """
        self.original_baseline = original_baseline.copy()
        self.adaptation_rate = adaptation_rate
        
        # Current adaptive baseline (starts as original)
        self.current_baseline = {
            'emg_envelope': {
                'mean': original_baseline['emg_envelope']['mean'],
                'std': original_baseline['emg_envelope']['std']
            },
            'emg_bandpowers': {
                'mean': original_baseline['emg_bandpowers']['mean'].copy() if isinstance(original_baseline['emg_bandpowers']['mean'], np.ndarray) else original_baseline['emg_bandpowers']['mean'],
                'std': original_baseline['emg_bandpowers']['std'].copy() if isinstance(original_baseline['emg_bandpowers']['std'], np.ndarray) else original_baseline['emg_bandpowers']['std']
            }
        }
        
        # Track consecutive REST predictions for adaptation
        self.consecutive_rest_count = 0
        self.min_rest_for_adaptation = 10  # Need 10 consecutive REST predictions before adapting
        self.rest_feature_buffer = deque(maxlen=20)  # Store recent REST features
    
    def update(self, prediction: str, features: dict):
        """
        Update adaptive baseline based on model predictions.
        Only adapts when REST is consistently predicted (handles fatigue).
        
        Args:
            prediction: Model prediction ('REST' or 'FIST')
            features: Current feature dictionary
        """
        if prediction == 'REST':
            self.consecutive_rest_count += 1
            
            # Store REST features for adaptation
            envelope_mean = float(np.mean(features['emg_envelope']))
            bandpowers = features['emg_bandpowers']
            
            self.rest_feature_buffer.append({
                'envelope_mean': envelope_mean,
                'bandpowers': bandpowers.copy() if isinstance(bandpowers, np.ndarray) else bandpowers
            })
            
            # Adapt baseline if we have enough consecutive REST predictions
            if (self.consecutive_rest_count >= self.min_rest_for_adaptation and 
                len(self.rest_feature_buffer) >= self.min_rest_for_adaptation):
                self._adapt_baseline()
        else:
            # FIST detected - reset consecutive REST count
            self.consecutive_rest_count = 0
    
    def _adapt_baseline(self):
        """
        Gradually adapt baseline towards current REST state.
        Uses exponential moving average to slowly shift baseline.
        """
        if len(self.rest_feature_buffer) < self.min_rest_for_adaptation:
            return
        
        # Compute average of recent REST features
        recent_envelopes = [f['envelope_mean'] for f in self.rest_feature_buffer]
        recent_bps = [f['bandpowers'] for f in self.rest_feature_buffer]
        
        avg_envelope = np.mean(recent_envelopes)
        avg_bandpowers = np.mean(recent_bps, axis=0) if len(recent_bps) > 0 else self.current_baseline['emg_bandpowers']['mean']
        
        # Adapt envelope mean (slowly shift towards current REST)
        current_mean = self.current_baseline['emg_envelope']['mean']
        new_mean = current_mean + self.adaptation_rate * (avg_envelope - current_mean)
        
        # Adapt envelope std (slightly increase if variability is higher)
        current_std = self.current_baseline['emg_envelope']['std']
        env_std_recent = np.std(recent_envelopes)
        new_std = current_std + self.adaptation_rate * (max(env_std_recent, current_std) - current_std)
        
        # Update baseline
        self.current_baseline['emg_envelope']['mean'] = max(new_mean, self.original_baseline['emg_envelope']['mean'] * 0.5)  # Don't go below 50% of original
        self.current_baseline['emg_envelope']['std'] = max(new_std, self.original_baseline['emg_envelope']['std'] * 0.5)
        
        # Adapt bandpowers
        if isinstance(avg_bandpowers, np.ndarray) and len(avg_bandpowers) >= 3:
            current_bp_mean = self.current_baseline['emg_bandpowers']['mean']
            if isinstance(current_bp_mean, np.ndarray):
                new_bp_mean = current_bp_mean + self.adaptation_rate * (avg_bandpowers - current_bp_mean)
                # Don't go below 50% of original
                original_bp_mean = self.original_baseline['emg_bandpowers']['mean']
                if isinstance(original_bp_mean, np.ndarray):
                    new_bp_mean = np.maximum(new_bp_mean, original_bp_mean * 0.5)
                self.current_baseline['emg_bandpowers']['mean'] = new_bp_mean
    
    def get_baseline(self) -> dict:
        """
        Get current adaptive baseline (for vs_rest computation).
        
        Returns:
            Current baseline dictionary
        """
        return self.current_baseline
    
    def reset(self):
        """Reset to original baseline (e.g., for new session)."""
        self.current_baseline = {
            'emg_envelope': {
                'mean': self.original_baseline['emg_envelope']['mean'],
                'std': self.original_baseline['emg_envelope']['std']
            },
            'emg_bandpowers': {
                'mean': self.original_baseline['emg_bandpowers']['mean'].copy() if isinstance(self.original_baseline['emg_bandpowers']['mean'], np.ndarray) else self.original_baseline['emg_bandpowers']['mean'],
                'std': self.original_baseline['emg_bandpowers']['std'].copy() if isinstance(self.original_baseline['emg_bandpowers']['std'], np.ndarray) else self.original_baseline['emg_bandpowers']['std']
            }
        }
        self.consecutive_rest_count = 0
        self.rest_feature_buffer.clear()


class SustainedFISTTracker:
    """
    Tracks sustained FIST with temporal smoothing to prevent REST flickering.
    Uses hysteresis: requires clear amplitude/bandpower differences from REST baseline
    before switching states. Once FIST is detected, requires multiple REST predictions
    to switch back (prevents flickering during sustained contractions).
    """
    
    def __init__(self, 
                 rest_baseline: dict,
                 fist_amplitude_threshold: float = 1.5,  # std above REST for FIST
                 fist_bandpower_threshold: float = 1.5,  # std above REST for FIST
                 rest_amplitude_threshold: float = 0.8,  # std above REST for REST (lower threshold)
                 rest_bandpower_threshold: float = 0.8,  # std above REST for REST (lower threshold)
                 min_rest_windows: int = 3):  # Need 3 consecutive REST to switch from FIST
        """
        Args:
            rest_baseline: REST baseline dictionary
            fist_amplitude_threshold: Amplitude threshold (std above REST) to detect FIST
            fist_bandpower_threshold: Bandpower threshold (std above REST) to detect FIST
            rest_amplitude_threshold: Amplitude threshold (std above REST) to confirm REST
            rest_bandpower_threshold: Bandpower threshold (std above REST) to confirm REST
            min_rest_windows: Minimum consecutive REST windows to switch from FIST to REST
        """
        self.rest_baseline = rest_baseline
        self.fist_amplitude_threshold = fist_amplitude_threshold
        self.fist_bandpower_threshold = fist_bandpower_threshold
        self.rest_amplitude_threshold = rest_amplitude_threshold
        self.rest_bandpower_threshold = rest_bandpower_threshold
        self.min_rest_windows = min_rest_windows
        
        # Current state
        self.current_state = 'REST'  # Start as REST
        self.consecutive_rest_count = 0
        
        # Feature history for temporal smoothing
        self.recent_amplitude = deque(maxlen=5)
        self.recent_bandpower = deque(maxlen=5)
    
    def update(self, envelope_mean: float, high_bandpower: float, model_prediction: str) -> str:
        """
        Update state based on features and model prediction.
        Uses hysteresis: clear thresholds for FIST, lower thresholds for REST.
        
        Args:
            envelope_mean: Current envelope mean
            high_bandpower: Current high-frequency bandpower
            model_prediction: Raw model prediction ('REST' or 'FIST')
            
        Returns:
            Final smoothed prediction ('REST' or 'FIST')
        """
        # Get REST baseline statistics
        rest_env_mean = self.rest_baseline['emg_envelope']['mean']
        rest_env_std = self.rest_baseline['emg_envelope']['std']
        rest_bp_mean = self.rest_baseline['emg_bandpowers']['mean'][2] if len(self.rest_baseline['emg_bandpowers']['mean']) > 2 else 0.0
        rest_bp_std = self.rest_baseline['emg_bandpowers']['std'][2] if len(self.rest_baseline['emg_bandpowers']['std']) > 2 else 1.0
        
        # Compute z-scores relative to REST baseline
        if rest_env_std > 1e-6:
            env_z_score = (envelope_mean - rest_env_mean) / rest_env_std
        else:
            env_z_score = 0.0
        
        if rest_bp_std > 1e-6:
            bp_z_score = (high_bandpower - rest_bp_mean) / rest_bp_std
        else:
            bp_z_score = 0.0
        
        # Store recent features
        self.recent_amplitude.append(env_z_score)
        self.recent_bandpower.append(bp_z_score)
        
        # Use median for robustness
        if len(self.recent_amplitude) >= 3:
            smoothed_env = float(np.median(list(self.recent_amplitude)))
            smoothed_bp = float(np.median(list(self.recent_bandpower)))
        else:
            smoothed_env = env_z_score
            smoothed_bp = bp_z_score
        
        # Hysteresis logic: different thresholds for FIST vs REST
        if self.current_state == 'FIST':
            # Currently in FIST state - require clear REST signal to switch
            # Both amplitude AND bandpower must be below REST threshold
            if (smoothed_env < self.rest_amplitude_threshold and 
                smoothed_bp < self.rest_bandpower_threshold):
                self.consecutive_rest_count += 1
                if self.consecutive_rest_count >= self.min_rest_windows:
                    # Confirmed REST - switch state
                    self.current_state = 'REST'
                    self.consecutive_rest_count = 0
            else:
                # Still FIST-like - reset REST counter
                self.consecutive_rest_count = 0
        else:
            # Currently in REST state - require clear FIST signal to switch
            # Either amplitude OR bandpower must be above FIST threshold
            if (smoothed_env > self.fist_amplitude_threshold or 
                smoothed_bp > self.fist_bandpower_threshold):
                # Clear FIST signal - switch immediately
                self.current_state = 'FIST'
                self.consecutive_rest_count = 0
    ...
    def reset(self):
        ...
        self.current_state = 'REST'
        self.consecutive_rest_count = 0
        self.recent_amplitude.clear()
        self.recent_bandpower.clear()


class SustainedContractionDetector:
    """
    Detects sustained muscle contractions (vs. short bursts).
    Uses both envelope amplitude AND bandpower to distinguish intentional sustained FIST from twitches/bursts.
    
    Enhanced with adaptive thresholding based on baseline characteristics.
    """
    
    def __init__(self, threshold_amplitude: float = 1.0,  # LOWERED from 2.0 for better sensitivity
                 threshold_bandpower: float = 1.0,  # LOWERED from 2.0 for better sensitivity
                 min_duration_seconds: float = 0.3,  # LOWERED from 0.5 for faster response
                 window_duration_ms: float = 200,
                 baseline_stats: Optional[dict] = None):
        """
        Args:
            threshold_amplitude: Envelope amplitude threshold (std above REST baseline)
            threshold_bandpower: Bandpower threshold (std above REST baseline)
            min_duration_seconds: Minimum duration to consider sustained
            window_duration_ms: Duration of each window in milliseconds
            baseline_stats: Optional REST baseline statistics for adaptive thresholding
        """
        self.base_threshold_amplitude = threshold_amplitude
        self.base_threshold_bandpower = threshold_bandpower
        self.threshold_amplitude = threshold_amplitude
        self.threshold_bandpower = threshold_bandpower
        self.min_duration_windows = int(min_duration_seconds * 1000 / window_duration_ms)
        
        # Track recent amplitude AND bandpower history
        self.amplitude_history = deque(maxlen=20)  # Last 20 windows (~4 seconds)
        self.bandpower_history = deque(maxlen=20)
        self.is_sustained = False
        
        # Adaptive thresholding
        if baseline_stats is not None:
            self._adapt_thresholds(baseline_stats)
    
    def _adapt_thresholds(self, baseline_stats: dict):
        """
        Adapt thresholds based on baseline characteristics.
        
        Args:
            baseline_stats: REST baseline statistics dictionary
        """
        env_mean = baseline_stats.get('emg_envelope', {}).get('mean', 50.0)
        env_std = baseline_stats.get('emg_envelope', {}).get('std', 10.0)
        bp_means = baseline_stats.get('emg_bandpowers', {}).get('mean', [0, 0, 100])
        bp_high_mean = bp_means[2] if len(bp_means) >= 3 else 100.0
        bp_stds = baseline_stats.get('emg_bandpowers', {}).get('std', [0, 0, 20])
        bp_high_std = bp_stds[2] if len(bp_stds) >= 3 else 20.0
        
        # Compute SNR for adaptivity
        env_snr = env_mean / (env_std + 1e-6)
        bp_snr = bp_high_mean / (bp_high_std + 1e-6)
        
        # Adjust thresholds based on signal quality
        # Lower thresholds for weak/noisy signals, higher for clean signals
        if env_mean < 10.0:  # Near-zero baseline (Device 2)
            self.threshold_amplitude = self.base_threshold_amplitude * 0.5  # Even more sensitive
        elif env_snr < 3.0:  # Very noisy
            self.threshold_amplitude = self.base_threshold_amplitude * 0.7  # More forgiving for noisy signals
        else:
            self.threshold_amplitude = self.base_threshold_amplitude
        
        if bp_high_mean < 50.0:  # Weak bandpower
            self.threshold_bandpower = self.base_threshold_bandpower * 0.5  # Even more sensitive
        elif bp_snr < 3.0:  # Very noisy
            self.threshold_bandpower = self.base_threshold_bandpower * 0.7  # More forgiving for noisy signals
        else:
            self.threshold_bandpower = self.base_threshold_bandpower
    
    def update(self, vs_rest_amplitude: float, vs_rest_bandpower: float) -> bool:
        """
        Update detector with new amplitude and bandpower measurements.
        
        Args:
            vs_rest_amplitude: Envelope amplitude relative to REST (z-score)
            vs_rest_bandpower: High bandpower relative to REST (z-score)
            
        Returns:
            True if sustained contraction is detected
        """
        self.amplitude_history.append(vs_rest_amplitude)
        self.bandpower_history.append(vs_rest_bandpower)
        
        # Count how many recent windows are above threshold for BOTH features
        above_threshold = 0
        for amp, bp in zip(self.amplitude_history, self.bandpower_history):
            # Both envelope AND bandpower must be elevated for true FIST
            if amp > self.threshold_amplitude and bp > self.threshold_bandpower:
                above_threshold += 1
        
        # Sustained if consistently above threshold for minimum duration
        self.is_sustained = above_threshold >= self.min_duration_windows
        
        return self.is_sustained
    
    def get_temporal_stability(self) -> tuple:
        """
        Compute temporal stability metrics for both amplitude and bandpower.
        
        Returns:
            Tuple of (amplitude_stability, bandpower_stability):
            Low values = sustained, high values = burst/dynamic
        """
        if len(self.amplitude_history) < 2:
            return (0.0, 0.0)
        
        # Compute coefficient of variation (std / mean) for both features
        amplitudes = np.array(list(self.amplitude_history))
        bandpowers = np.array(list(self.bandpower_history))
        
        mean_amp = np.mean(amplitudes)
        std_amp = np.std(amplitudes)
        mean_bp = np.mean(bandpowers)
        std_bp = np.std(bandpowers)
        
        cv_amp = std_amp / mean_amp if mean_amp > 0.1 else 0.0
        cv_bp = std_bp / mean_bp if mean_bp > 0.1 else 0.0
        
        return (cv_amp, cv_bp)
    
    def get_combined_score(self) -> float:
        """
        Get combined score from both amplitude and bandpower.
        Higher score = stronger FIST indication.
        
        Returns:
            Combined score (average of amplitude and bandpower z-scores)
        """
        if len(self.amplitude_history) == 0:
            return 0.0
        
        recent_amp = list(self.amplitude_history)[-1] if self.amplitude_history else 0.0
        recent_bp = list(self.bandpower_history)[-1] if self.bandpower_history else 0.0
        
        # Weighted average: bandpower is slightly more reliable for sustained contractions
        combined = (recent_amp * 0.4) + (recent_bp * 0.6)
        return combined
    
    def reset(self):
        """Reset the detector state."""
        self.amplitude_history.clear()
        self.bandpower_history.clear()
        self.is_sustained = False
...
class DevicePipeline:
    ...
    def __init__(self,
        ...
        # Adaptive baseline tracker (handles fatigue by adapting REST baseline)
        self.adaptive_baseline = None  # Will be initialized after baseline collection
        ...
        # Sustained FIST tracker (prevents flickering during sustained contractions)
        self.sustained_fist_tracker = None  # Will be initialized after baseline collection
        
        # Sustained contraction detection (relaxed for better sensitivity)
        self.sustained_fist_detector = SustainedContractionDetector(
            threshold_amplitude=1.2,  # Lowered: 1.2 std above REST (more sensitive)
            threshold_bandpower=1.2,  # Lowered: 1.2 std above REST (more sensitive)
            min_duration_seconds=0.2,  # Faster response: 200ms sustained
            window_duration_ms=200,     # Each window is 200ms
            baseline_stats=None  # Will be updated after baseline collection
        )
...
class InferenceEngine:
    ...
    def __init__(self,
        ...
        # Original: stride=25 (50ms), sequence=8 (400ms latency)
        # Optimized: stride=15 (30ms), sequence=5 (200ms latency) - faster response for safety
        self.stride = 15  # Reduced from 18 → 30ms between windows (faster response)
        self.sequence_length = 5  # Reduced from 6 → maintains temporal context with lower latency
        ...
        while pipeline.window_buffer.can_create_window() and windows_created < max_windows_per_batch:
            ...
            if pipeline.rest_baseline_collecting:
                ...
                if elapsed >= pipeline.rest_baseline_duration:
                    ...
                    # Initialize adaptive baseline tracker
                    pipeline.adaptive_baseline = AdaptiveBaselineTracker(
                        original_baseline=baseline,
                        adaptation_rate=0.01
                    )
                    
                    # Initialize sustained FIST tracker with adaptive thresholds
                    # If baseline has high variation (CV > 0.5), lower thresholds for better sensitivity
                    baseline_cv = cv_final
                    if baseline_cv > 0.8:  # High variation baseline
                        fist_amp_thresh = 1.0  # Lower threshold - more sensitive
                        fist_bp_thresh = 1.0
                    elif baseline_cv > 0.5:  # Moderate variation
                        fist_amp_thresh = 1.2
                        fist_bp_thresh = 1.2
                    else:  # Low variation - use standard thresholds
                        fist_amp_thresh = 1.5
                        fist_bp_thresh = 1.5
                    
                    pipeline.sustained_fist_tracker = SustainedFISTTracker(
                        rest_baseline=baseline,
                        fist_amplitude_threshold=fist_amp_thresh,  # Adaptive based on baseline quality
                        fist_bandpower_threshold=fist_bp_thresh,   # Adaptive based on baseline quality
                        rest_amplitude_threshold=0.8,
                        rest_bandpower_threshold=0.8,
                        min_rest_windows=3
                    )
                    
                    if baseline_cv > 0.5:
                        print(f"   Sustained FIST tracker: Adaptive thresholds (CV={baseline_cv:.3f}, thresh={fist_amp_thresh:.1f}σ)")
    ...
    def _run_inference(self, pipeline: DevicePipeline):
        ...
        # MOTION ARTIFACT REJECTION: If motion detected, force REST (no muscle activity)
        if motion_detected and raw_pred_class == 'FIST':
            # Motion artifact detected - reject FIST prediction
            raw_pred_class = 'REST'
            if self.debug and pipeline.total_sequences_processed % 10 == 0:
                print(f"[{pipeline.device_name}] Motion artifact rejected FIST (gyro={avg_gyro_mag:.0f}, accel={avg_accel_mag:.0f})")
        
        # TRUST MODEL FIRST: If model predicts FIST with decent confidence, use it
        # Only apply smoothing if model confidence is low or prediction is ambiguous
        if confidence >= 0.6:  # Model is confident
            final_prediction = raw_pred_class  # Trust the model
        elif pipeline.sustained_fist_tracker is not None:
            # Model confidence is low - use feature-based smoothing
            envelope_seq = sequence['emg_envelope']  # (seq_len, window_size)
            bandpowers_seq = sequence['emg_bandpowers']  # (seq_len, 3)
            
            # Use most recent window features
            if len(envelope_seq) > 0 and len(bandpowers_seq) > 0:
                envelope_mean = float(np.mean(envelope_seq[-1]))
                high_bandpower = float(bandpowers_seq[-1][2]) if len(bandpowers_seq[-1]) > 2 else 0.0
                
                # Apply temporal smoothing with feature-based thresholds
                smoothed_prediction = pipeline.sustained_fist_tracker.update(
                    envelope_mean=envelope_mean,
                    high_bandpower=high_bandpower,
                    model_prediction=raw_pred_class
                )
                final_prediction = smoothed_prediction
            else:
                final_prediction = raw_pred_class
        else:
            # No tracker yet - use raw prediction
            final_prediction = raw_pred_class
        
        # Track prediction history for safety monitoring
        pipeline.prediction_history.append(final_prediction)
        
        # Update adaptive baseline based on prediction
        if pipeline.adaptive_baseline is not None:
            # Get current features for adaptation
            sequence = pipeline.window_buffer.get_sequence()
            if sequence is not None:
                # Extract current features (use most recent window)
                current_features = {
                    'emg_envelope': sequence['emg_envelope'][-1] if len(sequence['emg_envelope']) > 0 else np.zeros(100),
                    'emg_bandpowers': sequence['emg_bandpowers'][-1] if len(sequence['emg_bandpowers']) > 0 else np.zeros(3)
                }
                pipeline.adaptive_baseline.update(final_prediction, current_features)
    ...
    def _compute_vs_rest_features(self, raw_features: dict, pipeline: DevicePipeline) -> np.ndarray:
        ...
        if (self.enable_rest_baseline and 
            ...
            # Use ADAPTIVE baseline if available (handles fatigue), otherwise use original
            if pipeline.adaptive_baseline is not None:
                rest_baseline = pipeline.adaptive_baseline.get_baseline()
            else:
    ...
    def start(self):
        ...
        print(f"   Motion cancellation: ENABLED (LMS with gyroscope)")
        ...
        print(f"      ✓ Dual-filter analysis (Butterworth+LMS amplitude + Wiener FFT frequency)")
        print(f"      ✓ Adaptive thresholds (adjusts to individual signal characteristics)")
        print(f"      ✓ Adaptive REST baseline (adapts to fatigue during session)")
        print(f"      ✓ Robust REST detection (requires 3 consecutive REST for reliable STOP)")
        print(f"      ✓ Fast FIST detection (immediate GO response for safety)")
        print(f"      → System adapts to fatigue - REST detected even if not returning to original baseline")
        print(f"      → More reliable STOP/GO commands for wheelchair control")
